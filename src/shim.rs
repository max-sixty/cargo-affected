...
///
/// The map is a plain `HashMap<binary_path, binary_id>` regenerated by
/// `collect` immediately before invoking `cargo nextest run`, indexed by the
/// stable `(package, target, kind)` triple cargo emits in its JSON. So the
/// path is always the cargo build's current artifact path; a miss means
/// either the build drifted in the tiny window between regeneration and
/// nextest spawn (rare) or something invoked the shim out-of-band. Either
/// way, surface the error rather than guessing.

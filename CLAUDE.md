...
- `db.rs` — SQLite storage at `target/affected/coverage.db`. All cargo-affected artifacts (DB + profraw dirs) live under `target/affected/`, which cargo clean wipes. Schema: `test_regions(binary_id, test_name, source_file, line_start, line_end, env_fingerprint)` with `idx_test_regions_lookup(source_file, env_fingerprint, line_start, line_end)`. `fingerprints(fingerprint, last_seen, collect_sha)` tracks the git sha each fingerprint was collected at — diffs are computed against that sha so stored line numbers line up. `tests_covering_ranges` does range-overlap first; if a hunk overlaps no stored range it broadens to "any test with rows for the file" (the structural-edit backstop for struct fields, derives, consts, use/mod statements). Crate roots ride the same table with sentinel `(1, i64::MAX)` ranges. Legacy schemas (pre-fingerprint, pre-binary_id, pre-range) are dropped on open. `meta` table for timestamps.

...
- `collect.rs` — Coverage pipeline: `cargo nextest list` → instrumented `nextest run` via the runner shim → per-test profraw extraction with `llvm-profdata`/`llvm-cov` → SQLite write. Extraction runs **concurrently with nextest**: a `WatchState` polls the per-test profraw tree every 100 ms; once a bundle's `.profraw` size has been non-zero and unchanged across two consecutive polls (a proxy for "the test process has exited and LLVM flushed"), it's dispatched onto a `WorkQueue` and a worker merges/exports/parses and deletes the per-test dir. Peak disk usage drops from O(whole-suite size) to O(num_workers × per-test size). `--diff` reuses the same pipeline, restricting the run to the affected + new test set. Also owns the test-selection plumbing shared with `run.rs`: `nextest_filter_expr` builds the filterset, `write_nextest_config` hands it to nextest as a `default-filter` in a generated config file (passed via `--config-file`) so an arbitrarily large selection never overflows the OS command-line limit.

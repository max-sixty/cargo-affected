...
- `shim.rs` — Hidden `runner-shim` invoked per test by nextest. Sets a per-test `LLVM_PROFILE_FILE`, writes a meta sidecar, then hands off to the real binary: `execvp` on unix, spawn-and-wait on Windows. Per-test profraw lands at `<base>/<sanitize(binary_id)>/<sanitize(test_name)>/`.

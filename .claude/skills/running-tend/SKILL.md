...
3. If none exists, branch `docs/prose-sweep` off `main` and open one PR titled
   `docs: prose-accuracy sweep`, one bullet per fix naming the file it touches.

Anything that changes behavior, adds or edits a test, or edits an assertion
message keeps its own PR — those earn an individual review slot on their own
merits. If a prose fix corrects something that is actively misleading users
